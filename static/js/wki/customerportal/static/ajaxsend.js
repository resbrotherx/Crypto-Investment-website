...
$(document).ready(function(){
	$('#sub_category').hide();
	var csrf = $("input[name=csrfmiddlewaretoken]").val();
	$("#category_of_issue").change(function(e){
		$('#sub_category').show();
		e.preventDefault();
		$.ajax({
			url:'/catajaxid/',
            ...
            dataType: "text",
			data: {
				ac_ns: $("#category_of_issue").val(),
	// 			'csrfmiddlewaretoken': csrf
			},
			success: function(data) {
				var mydatas = JSON.parse(data);
				// var mydata = JSON.parse(id);
				$('#sub_category').val($('#sub_category').val() + mydata[0].data);
				// $('#sub_category').val($('#sub_category').val() + mydatas.data);
			}
		})
	})
});

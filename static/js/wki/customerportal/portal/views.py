...
def unique_code(table,col):
    ...
    while unique:
        ...
        data = myclasss.Connect(host="188.166.99.136",user="postgres",password="",database="APLELIVE").select_single("SELECT id from %s where %s = '%s'" % (table,col,code))
        unique = len(data) > 0
    return code
...
def addCustomers153(request):
    ...
    print(data)
    if edit == False:
        use = connection = myclasss.Connect(host="188.166.99.136",database="APLELIVE",password="").insert(data)
    else:
        if request.POST.get('appli') != None:
            # print(request.POST.get('appli'))
            for val in json.loads(request.POST.get('appli')):
...
def connector():
    return myclasss.Connect(host='188.166.99.136',user='postgres',password='',database='APLELIVE',port='5432')
...
def juice_api(account_num,serialnumber,amount):
    ...
    #   wallet = 'wallet'
    # https://eedc.utiliflex.com/juice4/vending.php?mode=vend&submode=%s&account=%s&serialnumber=%s&amount=%s&return=json" % (wallet,account_num,serialnumber,amount)
    url = ""
    url = url.rstrip()
...
def recrut2(request,id):
    result = myclasss.Connect(host="188.166.99.136",database="APLELIVE",password="").select_single("SELECT id from hr_applicant where x_form_id = '%s'" % (id))
...
def recrut4(request,id):
    result = myclasss.Connect(host="188.166.99.136",database="APLELIVE",password="").select_single("SELECT id,name from hr_applicant where x_form_id = '%s'" % (id))
    if len(result) > 0:#if 'id'in result.keys():
...
def landingpage(request):
    ...
    # eedcbz.smartpowerbilling.com
    if 'Host' in headers.keys():
        if str(headers['Host']).strip() == '/':
...
def landin__page(request):
    return render(request,'report/landing_page/index.html')
